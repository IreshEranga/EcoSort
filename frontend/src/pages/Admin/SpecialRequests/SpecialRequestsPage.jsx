...
function SpecialRequestsPage() {
  ...
  useEffect(() => {
    const fetchSpecialRequests = async () => {
      try {
        ...
        // Filter for requests where collectStatus is 'Not Complete' or 'Assigned'
        const currentRequests = response.data.filter(request => 
          request.collectStatus === 'Not Complete' || request.collectStatus === 'Assigned'
        );
  ...
  useEffect(() => {
    // Filter current requests (Not Complete or Assigned)
    const filtered = specialRequests.filter(request =>
      request.collectStatus !== 'Completed' && // Ensure current requests have collectStatus 'Not Complete' or 'Assigned'
      (
    ...
    // Filter past requests (Completed)
      ...
      <div className="main-content" style={{ padding: '20px', textAlign: 'center' }}>
        ...
        <div style={{ marginTop: '30px', marginBottom: '40px' }}>
          <input
            ...
            style={{ padding: '8px', width: '450px', borderRadius: '10px' }}
          />
        ...
        <table className='special-requests-table' style={{ width: '100%', margin: '0 auto', borderCollapse: 'collapse', fontSize: '14px', marginBottom: '20px', marginTop: '20px' }}>      
          <thead> {/* Corrected from <head> to <thead> */}
            <tr> 
              ...
              <tr key={request._id}>
                ...
                <button
                  ...
                  style={{
                    borderRadius: '10px',
                    width: '90px',
                    backgroundColor: (request.paymentStatus === 'Done' || request.status === 'Pending' || request.amount !== 0) ? '#ccc' : '#00BFFF',
                    color: 'white'
                  }}
                >
